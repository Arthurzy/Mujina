...
public class ConfigurableSAMLProcessor extends SAMLProcessorImpl {
    ...
    public SAMLMessageContext sendMessage(SAMLMessageContext samlContext, boolean sign)
        ...
        SPSSODescriptor roleDescriptor = (SPSSODescriptor) samlContext.getLocalEntityMetadata().getRoleDescriptors()
                .get(0);
        AssertionConsumerService assertionConsumerService = roleDescriptor.getAssertionConsumerServices().stream()
                .filter(service -> service.isDefault()).findAny()
